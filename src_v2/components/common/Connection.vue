    ...
    <div class="content">
      ...
      <div class="op">
        <span
          ...
          class="icon"
        ><img
            :src="require('image/new_folder.png')"
            v-on:click="cFolder"
          /></span>
        <span
          v-if="item.type==='folder'"
          class="icon"
        ><img
            :src="require('image/new.png')"
            v-on:click="$emit('c-connect',index)"
          /></span>
        <span class="icon"><img :src="require('image/edit.png')" /></span>
        <span class="icon"><img
            :src="require('image/del.png')"
            v-on:click="delConnectionPack"
          /></span>
      </div>
    </div>
    <div
      class="child"
      v-if="item.type==='folder' && item.child.length && item.show_child"
    >
      <Connection
        v-for="(child,i) in item.child"
        v-bind:key="i"
        v-bind:index="index+'|'+i"
        v-bind:item="child"
        v-on:c-folder="$emit('c-folder',$event)"
        v-on:c-connect="$emit('c-connect',$event)"
      />
    </div>
    <ConfirmWindow
      v-if="show_confirm"
      ...
      v-on:confirm-cancel="confirmDelCancel"
      v-on:confirm-sure="confirmDelSure"
    />
  </div>
</template>
...
export default {
  ...
  computed: {
    show_icon() {
      if (this.item.type === "folder" && this.item.show_child === true) {
  ...
  methods: {
    ...
    toggleShowChild() {
      ...
      this.$store.commit("AStore/toggleChild", this.index);
    },
    cFolder() {
