  ...
  <div class="connections" :style="'width:'+width+'px'">
    <div class="content">
      <div class="button" ref="button">
        <IButton val="add_connection" v-on:b-on="showType" color="#031234" />
      </div>
      <div class="new" v-show="type_show" v-on:mouseleave="hideType" ref="type">
        <div style="height:37px;"></div>
        <div class="new-type">
          <div class="new-folder" v-on:click="showAddFolderWindow('')">
            <img :src="require('image/new_folder.png')" />
          </div>
          <div class="new-connect" v-on:click="showAddConnectionWindow">
        ...
        <Connection v-for="(connection,i) in connections" v-bind:key="i" v-bind:index="i+''" v-bind:item="connection"
          top='1' v-on:c-folder="showAddFolderWindow($event)" v-on:c-connect="()=>{}" />
      </div>
    </div>
    <span class="handle" @mousedown="startDrag"><img :src="require('image/drag.png')" /></span>
  ...
  export default {
    ...
    methods: {
      ...
      async createFolder(folder) {
        await this.$store.dispatch('AStore/addConnection', {
